...
class Ship:
    ...
    def shoot(self):
        global a_missile   
        missile_vel = [self.vel[0]*30+self.forward[0],self.vel[1]*30+self.forward[1]]
        missile_pos = [self.pos[0]+45*self.forward[0],self.pos[1]+self.forward[1]]        
        a_missile = Sprite([self.pos[0]+45, self.pos[1]],
...
def draw(canvas):
    ...
    canvas.draw_image(debris_image, center, size, (wtime + WIDTH / 2, HEIGHT / 2), (WIDTH, HEIGHT))
    canvas.draw_text('Score: '+str(score), [50, 50], 20, 'Blue')
    canvas.draw_text('Lives: '+str(lives), [650, 50], 20, 'Blue')
...
def rock_spawner():
    global a_rock
    x_position = random.randint(1, 5)
    y_position = random.randint(1, 4)    
    ...
    a_rock = Sprite([WIDTH / x_position, HEIGHT / y_position], 
...
a_rock = Sprite([WIDTH / 3, HEIGHT / 3], [0.5, 0.5], 0, 0.1, asteroid_image, asteroid_info)
